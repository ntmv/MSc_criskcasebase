...
# We generate data from two weibull-cause specific hazards with the first casue having a higher incidence than the second cause
n <- 400
p <- 20
...
# We set the parameters so that approximately 25% are censored, and we have 52% cause 1 incidence
...
# Visualize cumulative incidence function
plot(cif)
#################################################################
...
tic()
# This cross-validation function can take a while, I have parallelized it with mclapply so it should run faster but check to adjust number of cores etc.
...
# Case-base fits 
# We try out different lambdas in the grid just to compare performance
# Lambda.min
...
# Calculate specificity, sensitivity and MCC
...
###################################################################################
# Put all variable selection results together
