...
path <- "~/Desktop/results_pgn_iid_sparse"
...
# Read in all the output read files 
path <- "~/Desktop/results_pgn_cor_sparse"
...
pgn_res_stats <- pgn_res %>%
  group_by(Model, cor_setting, sparse_setting) %>%
  summarise(mean_sp = mean(Sp), mean_se = mean(Se), 
            ...
            max_sp = max(Sp), max_se = max(Se), perc_se = sum(Se > 0.60)/n(), 
            perc_sp = sum(Sp > 0.60)/n())
