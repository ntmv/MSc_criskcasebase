########################## Multinomial simulation for relaxed LASSO #######################
# Libraries 
library(casebase)
...
Results <- replicate(N, {
  tryCatch({
    ...
    ############################ glmnet model #############################
    # If glmnet model does not fit, continue with simulation
    tryCatch({
      ...
      # Test multi-deviance
      dev_glmnet <- multi_deviance_final(data = dfM_test_list, fit_object = glmnet_coefs)
    }, 
    error = function(e) {
      print(e)
    ...
    fit.mtool.relaxed_no_penalization <-
      multinomial_casebase_relaxed_lasso(train = dfM_train, gamma = 0, seed = seed)
    ...
    fit.mtool.relaxed_small_penalization <-
      multinomial_casebase_relaxed_lasso(train = dfM_train, gamma = 0.00005, seed = seed)
    ...
    fit.mtool.relaxed_significant_penalization <-
      multinomial_casebase_relaxed_lasso(train = dfM_train, gamma = 0.001, seed = seed)
    
    cat("Lambda min of significant penalization fit: ", fit.mtool.relaxed_significant_penalization$lambda.min)
...
write_csv(as.data.frame(sim_coefficient_results), file = glue("simulation_final/multinomial/single_run_results/coefficients/results_multinomial_{runif(1)}.csv"))
write_csv(as.data.frame(sim_deviance_results), file = glue("simulation_final/multinomial/single_run_results/deviances/deviances_multinomial_{runif(1)}.csv"))
