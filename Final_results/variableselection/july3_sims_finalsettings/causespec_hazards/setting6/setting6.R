########################### n = 400, p = 1000, Tp = 20 simulation cause 2 > cause 1######################
library(casebase)
...
beta2 <- c(rep(0, p))
nu_ind <- seq(num_true)
beta1[nu_ind] <- c(-1.3, -0.79, -0.5, -1.35, -1.29, 0.84, 1.5, 1.8, 0.96, 0.75, -1.3, -0.79, -0.5, -1.35, -1.29, 0.84, 1.5, 1.8, 0.96, 0.75)
beta2[nu_ind] <- c(-0.86, -0.55, -0.4, -1.35, -1.09, -0.84, -1.5, -1.8, -0.96, -0.75, 1.3, 0.79, 0.5, 1.35, 1.29, 0.66, 1.3, 1.4, 0.55, 0.65)

# Simulate data
sim.data <- cause_hazards_sim(n = n, p = p, nblocks = 4, 
                              beta1 = beta1, beta2 = beta2, rate_cens = 4.5, 
                              h1 = 0.30, h2 = 0.55, gamma1 = 1.5, gamma2 = 1.5)
...
write.csv(Res, file = glue("setting6{runif(1)}.csv"))
