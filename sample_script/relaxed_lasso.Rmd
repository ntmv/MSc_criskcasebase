...
coefficient_values = rbind(coefficient_values, c(0, lapply(coef(fit_lasso_cv)[-1], function(x) round(x, 3))))
...
for(i in (1: length(all_coef_fit_lasso_cv))) {
  ...
  coef_fit_OLS_on_LASSO_cv_subset = coef(fit_OLS_on_LASSO_cv_subset)[-1]
...
colnames(coefficient_values) = coefficient_names
model_fit_labels = c("glmnet relax = TRUE, CV, lambda min", "glmnet relax = TRUE, CV, lambda 1se", "glmnet lasso noCV, then CV-OLS fit on selected predictors from every lambda", "glmnet lasso CV", "glmnet lasso CV, then CV-OLS fit on selected predictors from every lambda")
