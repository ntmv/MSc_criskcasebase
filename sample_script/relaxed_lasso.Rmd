---
title: "Relaxed Lasso Glmnet Linear Regression Implementation"
...
Per meeting on June 15th, tried fitting LASSO in first iteration without cross validation, took sets of predictors selected with each lambda value tried, then fit OLS on each of those sets, taking the OLS fit with the lowest CV-MSE as the best fit.

``` {r lasso-then-ols-all-predictors, echo = FALSE, warnings = FALSE}
results = myRelaxed(x_train, y_train, FALSE)
results_cv = myRelaxed(x_train, y_train, TRUE)


myRelaxed_coefficient_row = formatCoefficientTableRow(results$coefficients, col_names)
coefficient_values = rbind(coefficient_values, c(0, lapply(myRelaxed_coefficient_row, function(x) round(x, 3))))

myRelaxed_cv_coefficient_row = formatCoefficientTableRow(results_cv$coefficients, col_names)
coefficient_values = rbind(coefficient_values, c(0, lapply(myRelaxed_cv_coefficient_row, function(x) round(x, 3))))
colnames(coefficient_values) = coefficient_names
```
...
``` {r lasso-cv-then-ols-all-predictors, echo = FALSE, warnings = FALSE}
# results_cv = myRelaxed(x_train, y_train, TRUE)
# 
# col_names = rownames(as.data.frame.matrix(coef(cv_fit_relaxed)))
# myRelaxed_cv_coefficient_row = formatCoefficientTableRow(results_cv$coefficients, col_names)
# 
# coefficient_values = rbind(coefficient_values, c(0, lapply(myRelaxed_cv_coefficient_row, function(x) round(x, 3))))
# colnames(coefficient_values) = coefficient_names
...
``` {r inspect}
fit_lasso_cv = cv.glmnet(x_train, y_train, family="gaussian", keep=TRUE, alpha=1)
...
colnames(coefficient_values) = coefficient_names
model_fit_labels = c("glmnet relax = TRUE, CV, lambda min", "glmnet relax = TRUE, CV, lambda 1se", "glmnet lasso noCV, then CV-OLS fit on selected predictors from every lambda", "glmnet lasso CV, then CV-OLS fit on selected predictors from every lambda")
...
pred_fit_lasso_cv = predict(fit_lasso_cv, newx = x_test)
...
mse_lasso_cv <- sum((pred_fit_lasso_cv - y_test)^2)
...
MSEs[, 2] = c(mse_relaxed_min, mse_relaxed_1se, mse_lasso_then_OLS_all_predictors, mse_lasso_cv, mse_lasso_cv_then_OLS_all_predictors)
...
plot(fit_lasso_cv, main = "glmnet lasso cv")
