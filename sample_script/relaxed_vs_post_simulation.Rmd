...
## Fit models (one iteration)

``` {r different-models-fits, echo=FALSE}
fitAll = function(train_data, response, print_time) {
  train_data = training_data$x_train
  response = training_data$y_train
  print_time = FALSE
  ...
  coef_lasso_same_lambda = as.list(coef(fit_Y1))
  non_zero_coef_lasso_then_OLS_same_lambda = as.list(coef_lasso_same_lambda[coef_lasso_same_lambda != 0])
  ...
  #print(coef_lasso_same_lambda)
  #print(non_zero_coef_lasso_then_OLS_same_lambda)
  #print(non_zero_coef_lasso_then_OLS_same_lambda_indeces)
  ...
  # new_train_data = cbind(rep(1, times = nrow(new_train_data)), new_train_data)
  colnames(new_train_data) = selected_coefficient_names
  # colnames(new_train_data) = c("(Intercept)", selected_coefficient_names)
...
computeMSEs = function(resulting_fits, test_data) {
  ...
  # print(pred_fit_post_lasso_min)
  # print(pred_fit_post_lasso_1se)
...
## Format coefficient table (one iteration)

``` {r format-coefficient-table, echo=FALSE}
