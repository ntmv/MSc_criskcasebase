...
multinom.relaxed_lasso_nnet <- function(train, regularization = 'elastic-net', lambda_max = NULL, alpha = 1, nfold = 10, 
...
###########################################################
#' relaxed LASSO function for mtool 
multinom.relaxed_lasso_mtool <- function(train, regularization = 'l1', lambda_max = NULL, alpha = 1, nfold = 10, 
                                         constant_covariates = 2, initial_max_grid = NULL, precision = 0.001, epsilon = .0001, grid_size = 100, plot = FALSE, 
...
# IGNORE
#' #' relaxed LASSO function for mtool
#' multinom.relaxed_lasso_err <- function(train, regularization = 'l1', lambda_max = NULL, alpha = 1, nfold = 10,
...
#'   constant_covariates = 2; initial_max_grid = NULL; precision = 0.001; epsilon = .1; grid_size = 100; plot = FALSE;
#'   ncores = parallelly::availableCores(); seed = 2023; train_ratio = 20; lambda_max = 0.2; gamma = 0.009
#' 
#'   lambdagrid <- rev(round(exp(seq(log(lambda_max), log(lambda_max*epsilon), length.out = grid_size)), digits = 10))
#' 
#'   # Create folds
...
#' 
#'                  # Calculate the resulting deviance on the test set
#'                  dev = multi_deviance(data = test_cv, fit.mtool.subset)
#' 
#'                  # Fit a penalized model on entire dataset (all covariates, all observations) during last fold of cv
#'                  if(i == nfold) {
#'                    # Subset columns of covariates
#'                    train$covariates <- train[, colnames(train) %in% non_zero_cov_names]
#' 
#'                    # Fit a multinomial model on subsetted data with a gamma penalization
#'                    fit.all.data = fit_model_lasso(train, regularization = 'l1',
#'                                               lambda = gamma, alpha = alpha, unpen_cov = 1)
#'                    num_non_zero_cov = length(union(which(fit.all.data$coefficients[, 1] != 0),
#'                                                    which(fit.all.data$coefficients[, 2] != 0)))
#' 
...
plot_post_relaxed.multinom_test <- function(cv_object) {
  ...
  p <- ggplot(plot.dat.p, aes(log(lambdagrid), mean.dev)) + geom_point(colour = "red", size = 3) + theme_bw() + 
    ...
    # geom_text(stat="count", aes(label=..count..), y=cv_object$coef_grid, size=5) +
  ...
  #                          upper = mean_dev +row_stdev, lower = mean_dev - row_stdev)
  # 
  # g <- ggplot(plot.dat.g, aes(cv_object$coef_grid, mean.dev)) + geom_point(colour = "red", size = 3) + theme_bw() +
  #   geom_errorbar(aes(ymin= lower, ymax=upper), width=.2, colour = "grey") +
  #   labs(x = "log(lambda)", y = "Multinomial Deviance")  +
  #   geom_vline(xintercept = log(cv_object$lambda.min), linetype = "dotted", colour = "blue")
  # 
...
add_secondary_axis <- function(p, positions, labels) {
  ...
  y_data <- ggplot_build(p)$data[[1]]$y
  y_limit <- max(y_data) + 0.1 * diff(range(y_data))
  ...
  for (i in seq_along(positions)) {
    ...
    p <- p + 
      annotation_custom(
        grob = grid::segmentsGrob(arrow = grid::arrow(type = "open", ends = "first", length = unit(0.2, "inches"))),
        xmin = positions[i], xmax = positions[i],
        ymin = y_limit - (0.05 * diff(range(y_data))), ymax = y_limit
      )
  }
...
multinom.relaxed_lasso <- function(train, regularization = 'l1', lambda_max = NULL, alpha = 1, nfold = 10, 
                                   constant_covariates = 1, initial_max_grid = NULL, precision = 0.001, epsilon = .0001, grid_size = 100, plot = FALSE, 
                                   ncores = parallelly::availableCores(), seed = NULL, train_ratio = 20, gamma = 0.009) {
  print("RELAXED")
  p = ncol(train)
      ...
      mclapply(lambdagrid,
               function(lambda_v) {
                 ...
                 if(i == nfold) {
                   ...
                   fit.all.data = fit_model_lasso(train, regularization = 'l1',
                                              lambda = gamma, alpha = alpha, unpen_cov = 1)
                   num_non_zero_cov = length(union(which(fit.all.data$coefficients[(1:p_new), 1] != 0),
                                                   which(fit.all.data$coefficients[(1:p_new), 2] != 0)))
